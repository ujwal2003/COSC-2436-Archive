...
string searchData(string arr[], int size, string target) {
	static int level = 0;
	
	if(size==1) {
		if(dataFound(toLowercase(arr[0]), toLowercase(target))) {
			return "Level " + to_string(level-2) + ": " + arr[0] + "\n";
		} else {
		...
		if(size%2==0) {
			...
			level++;
			return searchData(arr1, halfSize, target) + searchData(arr2, halfSize, target);
		} else {
			...
			level++;
			return searchData(bigArr, bigSize, target) + searchData(smallArr, smallSize, target);
		}
	}
}
...
int main(int argc, char *argv[]) {
	...
	//printArray(records, numRecords(input));
	//cout << searchData(records, numRecords(input), target);
	ofs << searchData(records, numRecords(input), target);
	ofs.flush();
